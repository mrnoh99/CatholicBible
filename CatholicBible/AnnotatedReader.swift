        ...
        VStack(spacing: 0) {
            ...
            .onAppear {
                ...
                if chapter > 0 {
                    ...
                    // 상위 ReaderView의 장 바인딩이 대기 이동을 이미 이 장으로 계산해 보여주고
                    // 있을 수 있다(책이 바뀌며 이 화면이 새로 생성된 경우). 그래도 setChapter는
                    // 항상 호출해 그 값을 "이 책 것"으로 실제 저장해야 한다 — 그래야 아래에서
                    // pendingChapter를 지운 뒤에도 다음 렌더에서 장이 0으로 되돌아가지 않는다.
                    // 절 스크롤·강조 소비도 여기서 마저 처리한다.
...
struct IntroDetailView: View {
    ...
    private var bodyText: some View {
        ...
        if intro.body.isEmpty {
            ...
            let normalized = ScriptureRefNormalizer.normalize(intro.body,
                                                           currentBookID: intro.bookID ?? "",
                                                           chapter: 0)
            displayText = ScriptureRefNormalizer.addVerseMarkers(normalized)
        }
