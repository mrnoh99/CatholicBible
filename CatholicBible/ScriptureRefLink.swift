...
enum ScriptureRef {
    ...
    /// 범위 지원: "1,1절-5절" (같은 장), "1,1절-2,4절" (다른 장)
    /// 그룹: 1=책이름(선택적, 1-3 숫자 접두사 포함) 2=장 3=절 4=대시뒤 장(선택적) 5=대시뒤 절 6=점뒤 절 7=점뒤 대시 절
    private static let koreanRegex = try? NSRegularExpression(
        pattern: "([1-3]?[가-힣]+(?:\\s[가-힣]+)*(?:복음|서간|기|편)?)?\\s*(\\d{1,3})[:,](\\d{1,3})절(?:[의에서가에]*)?(?:\\)?[–-](?:(\\d{1,3}),)?(\\d{1,3})절(?:[의에서가에]*)?)?(?:\\)?\\.(\\d{1,3})절(?:[의에서가에]*)?(?:[–-](\\d{1,3})절(?:[의에서가에]*)?)?)?")
