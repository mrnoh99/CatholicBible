...
- **iPad 2단 / iPhone 1단**: iPad(가로 넓은 화면)에서는 **두 열을 나란히**
  봅니다. 각 열이 **판본·책·장을 따로** 고르므로 — 같은 성경을 서로 다른
  곳에 펼치거나, 다른 성경을 나란히 비교하는 것이 모두 됩니다(툴바의 2단
  토글로 켜고 끔). iPhone에서는 한 열만 봅니다.
- **리더**: 열마다 판본·책 선택 메뉴 + 하단 장 이동 바(◀ 슬라이더 ▶ ·
  장 번호 탭으로 바로 이동)
...
- **절 노트 (판본 공통, 멀티미디어)**: 절마다 노트를 답니다(판본 무관, 같은 절
  ...
  탭하면 바로 열립니다.
- **검색**: 현재 판본의 수록 본문 전체에서 구절 검색, 결과 절로 이동해 강조
