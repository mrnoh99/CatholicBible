...
- **iPad 2단 / iPhone 1단**: iPad(가로 넓은 화면)에서는 **두 판본을 나란히**
  볼 수 있고(각 열에서 판본을 따로 선택, 툴바의 2단 토글로 켜고 끔),
  iPhone에서는 한 판본만 봅니다. 장은 두 열이 함께 움직입니다.
- **리더**: 하단 장 이동 바(◀ 슬라이더 ▶), 장 번호 그리드로 바로 이동
