#!/usr/bin/env python3
"""
공동번역성경 PDF에서 헤딩(소제목)을 추출하는 스크립트 - v4 폰트 기반 버전
폰트 메타데이터를 활용한 고정확 추출
"""

import pdfplumber
import re
import json
from pathlib import Path
from typing import Dict, List, Tuple, Optional

# 책 ID 매핑
BOOK_MAPPING = {
    '창세기': 'gn', '출애굽기': 'ex', '레위기': 'lv', '민수기': 'nm', '신명기': 'dt',
    '여호수아': 'jos', '사사기': 'jgs', '룻기': 'rt', '사무엘상': '1sm', '사무엘하': '2sm',
    '열왕기상': '1kgs', '열왕기하': '2kgs', '역대상': '1chr', '역대하': '2chr',
    '에스라': 'ezr', '느헤미야': 'neh', '에스더': 'est', '욥기': 'jb', '시편': 'ps',
    '잠언': 'prv', '전도서': 'qoh', '이사야': 'is', '예레미야': 'jr', '애가': 'lm',
    '에제키엘': 'ez', '다니엘': 'dn', '호세아': 'hs', '요엘': 'jl', '아모스': 'am',
    '오바디야': 'ob', '요나': 'jnh', '미가': 'mi', '나훔': 'na', '하박국': 'hb',
    '스바냐': 'zph', '학개': 'hg', '스가랴': 'zc', '말라기': 'ml',
    '마태오': 'mt', '마르코': 'mk', '루가': 'lk', '요한': 'jn',
    '사도행전': 'act', '로마': 'rom', '고린토전서': '1cor', '고린토후서': '2cor',
    '갈라디아': 'gal', '에페소': 'eph', '필리피': 'phil', '골로새': 'col',
    '데살로니가전서': '1thes', '데살로니가후서': '2thes', '디모테오전서': '1tm',
    '디모테오후서': '2tm', '디토': 'tit', '필레몬': 'phlm', '히브리': 'heb',
    '야고보': 'jas', '베드로전서': '1pt', '베드로후서': '2pt', '요한1서': '1jn',
    '요한2서': '2jn', '요한3서': '3jn', '유다': 'jud', '요한묵시록': 'rv'
}

class NCBHeadingExtractorV4:
    def __init__(self):
        self.headings: Dict[str, Dict[int, Dict[int, str]]] = {}
        self.current_book = None
        self.current_chapter = None
        self.last_verse = 0

        # 폰트 임계값 (공동번역성경 구조)
        self.heading_font = "NanumGothic"  # 헤딩 폰트
        self.heading_size = 7.0  # 헤딩 크기
        self.heading_tolerance = 0.5  # 크기 오차 범위

    def extract_from_pdf(self, pdf_path: str) -> Dict:
        """PDF 파일에서 헤딩 추출 (폰트 기반)"""
        print(f"📖 PDF 처리 중: {pdf_path}")

        with pdfplumber.open(pdf_path) as pdf:
            print(f"   총 {len(pdf.pages)}페이지\n")
            for i, page in enumerate(pdf.pages):
                self._parse_page_by_font(page)
                if (i + 1) % 5 == 0 or (i + 1) == len(pdf.pages):
                    print(f"   ✓ {i+1}/{len(pdf.pages)} 페이지 처리")

        return self.headings

    def _parse_page_by_font(self, page):
        """폰트 메타데이터를 사용하여 페이지 파싱"""
        chars = page.chars
        if not chars:
            return

        # 문자를 라인별로 그룹화
        lines = self._group_chars_by_line(chars)

        for line_chars in lines:
            if not line_chars:
                continue

            text = ''.join([c['text'] for c in line_chars])

            # 책 이름 감지
            for book_name, book_id in BOOK_MAPPING.items():
                if book_name in text and '제' in text and '장' in text:
                    chapter_match = re.search(r'제(\d+)장', text)
                    if chapter_match:
                        self.current_book = book_id
                        self.current_chapter = int(chapter_match.group(1))
                        if book_id not in self.headings:
                            self.headings[book_id] = {}
                        if self.current_chapter not in self.headings[book_id]:
                            self.headings[book_id][self.current_chapter] = {}
                        self.last_verse = 0
                    break

            # 절 번호 감지
            verse_match = re.match(r'^(\d+)\s+', text)
            if verse_match and self.current_chapter is not None:
                self.last_verse = int(verse_match.group(1))

            # 헤딩 감지: 폰트가 NanumGothic @ 7.0pt인 경우
            if self._is_heading_by_font(line_chars):
                if (self.current_book and self.current_chapter is not None and
                    self.last_verse > 0):

                    # 절 번호를 텍스트에서 제거
                    heading_text = re.sub(r'^\d+\s+', '', text).strip()

                    if heading_text and self.current_book in self.headings:
                        if self.current_chapter in self.headings[self.current_book]:
                            if self.last_verse not in self.headings[self.current_book][self.current_chapter]:
                                self.headings[self.current_book][self.current_chapter][self.last_verse] = heading_text

    def _group_chars_by_line(self, chars: List) -> List[List]:
        """문자를 Y 좌표 기준으로 라인별로 그룹화"""
        if not chars:
            return []

        lines = []
        current_line = []
        current_y = chars[0]['top']

        for char in chars:
            # Y 좌표 변화 감지 (새로운 줄)
            if abs(char['top'] - current_y) > 2:
                if current_line:
                    lines.append(current_line)
                current_line = [char]
                current_y = char['top']
            else:
                current_line.append(char)

        if current_line:
            lines.append(current_line)

        return lines

    def _is_heading_by_font(self, line_chars: List) -> bool:
        """라인이 헤딩인지 폰트 정보로 판단"""
        if not line_chars:
            return False

        # 라인의 주요 폰트 확인 (첫 문자의 폰트)
        first_char = line_chars[0]
        fontname = first_char.get('fontname', '')
        size = first_char.get('size', 0)

        # NanumGothic @ 7.0pt 확인
        is_gothic = 'NanumGothic' in fontname or 'gothic' in fontname.lower()
        is_correct_size = abs(size - self.heading_size) < self.heading_tolerance + 0.1

        # 대부분의 문자가 같은 폰트/크기여야 함
        same_font_count = sum(1 for c in line_chars
                             if (('NanumGothic' in c.get('fontname', '') or 'gothic' in c.get('fontname', '').lower())
                                 and abs(c.get('size', 0) - self.heading_size) < 0.5))
        consistency = same_font_count / len(line_chars) if line_chars else 0

        return is_gothic and is_correct_size and consistency > 0.7

    def save_to_json(self, output_path: str):
        """추출한 헤딩을 JSON으로 저장"""
        output_data = {
            "headings": {
                book_id: {
                    str(chapter_num): {
                        str(verse_num): text
                        for verse_num, text in verses.items()
                    }
                    for chapter_num, verses in chapters.items()
                }
                for book_id, chapters in self.headings.items()
            }
        }

        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(output_data, f, ensure_ascii=False, indent=2)

        print(f"\n💾 저장 완료: {output_path}")

    def print_summary(self):
        """추출 결과 요약"""
        print("\n" + "="*70)
        print("📊 추출 결과 요약")
        print("="*70)

        total_books = len(self.headings)
        total_chapters = sum(len(c) for c in self.headings.values())
        total_headings = sum(sum(len(v) for v in c.values()) for c in self.headings.values())

        print(f"📚 총 {total_books}권 | 🔢 {total_chapters}장 | 📝 {total_headings}개 헤딩\n")

        for book_id in sorted(self.headings.keys()):
            chapters = self.headings[book_id]
            if chapters:
                chapter_count = len(chapters)
                heading_count = sum(len(v) for v in chapters.values())
                avg_headings = heading_count / chapter_count if chapter_count > 0 else 0
                print(f"▶ {book_id.upper()}: {chapter_count}장, {heading_count}개 헤딩 (장당 {avg_headings:.1f}개)")

        print()


def main():
    import sys

    if len(sys.argv) < 2:
        print("📖 공동번역성경 헤딩 추출기 v4 (폰트 기반)\n")
        print("사용법:")
        print("  python3 extract_ncb_headings_v4.py <pdf_파일> [출력_파일.json]\n")
        print("예시:")
        print("  python3 extract_ncb_headings_v4.py 공동번역성경.pdf ncb_headings.json")
        return

    pdf_file = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else "ncb_headings.json"

    if not Path(pdf_file).exists():
        print(f"❌ 오류: 파일을 찾을 수 없습니다: {pdf_file}")
        return

    extractor = NCBHeadingExtractorV4()
    extractor.extract_from_pdf(pdf_file)
    extractor.print_summary()
    extractor.save_to_json(output_file)

    print(f"✨ 생성된 JSON을 다음과 같이 사용할 수 있습니다:")
    print(f"   python3 merge_ncb_headings.py --to-notes {output_file} NcbNotes.json")


if __name__ == "__main__":
    main()
